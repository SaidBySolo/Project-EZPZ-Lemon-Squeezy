...
class QnA(commands.Cog):
    ...
    async def on_message(self, message):
        if not message.guild and message.author.id != 657604407021666304:
            ...
            qtu.set_footer(text=f"문의일:(UTC) {str(now.year)}년 {str(now.month)}월 {str(now.day)}일 {str(now.hour)}시 {str(now.minute)}분 {str(now.second)}초")
            await message.channel.send("문의가 정상적으로 처리되었습니다.", delete_after= 5)
    ...
    async def 답변(self, ctx, did, *, ans):
        ...
        atu.set_footer(text=f"답변일:(UTC) {str(now.year)}년 {str(now.month)}월 {str(now.day)}일 {str(now.hour)}시 {str(now.minute)}분 {str(now.second)}초")
